...
impl Endpoint {
    ...
    /// Return the address in the format of `ip:port`, or `[ip]:port` for an
    /// IPv6 address, which is the form [`Self::parse`] reads back.
...
mod tests {
    ...
    fn test_endpoint_parse_ipv6() -> anyhow::Result<()> {
        ...
        assert_eq!("[::1]:1234", Endpoint::parse("[::1]:1234")?.to_address());
